...
def infoArtifact(artifactName):
    ...
    if artifactInfo[2] != "":
        embed = discord.Embed(title=artifactInfo[0], description=artifactInfo[2][0:250:] + "...")
    else:
    ...
    if artifactInfo[9] != "":
        embed.add_field(name="Power Level",value=artifactInfo[9], inline=True)

    if artifactInfo[6] != "":
        tagString = ""
        for tag in artifactInfo[6].split(","):
            if len(tag.split("|")) >= 2:
                tagString = tagString + tag.split("|")[0] + " (" + tag.split("|")[1] + "), "
            else:
                tagString = tagString + tag + ", "
    ...
    if artifactInfo[5] != "":
        wielderString = ""
        for person in artifactInfo[5].split("|"):
